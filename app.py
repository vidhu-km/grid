...
def load_data():
    ...
    well_df_out = well_df.merge(section_df[["Section", SECTION_OOIP_COL, SECTION_ROIP_COL]], on="Section", how="left")
    ...
    existing_wells = gpd.GeoDataFrame(pd.concat([lines_with_uwi, points_only], ignore_index=True), geometry="geometry", crs=lines.crs)
    proximal_wells = gpd.GeoDataFrame(existing_wells.merge(well_df_out, on="UWI", how="inner"), geometry="geometry", crs=existing_wells.crs)
    proximal_wells["_midpoint"] = proximal_wells.geometry.apply(midpoint_of_geom)

    return (lines, points, grid, inv, land, well_df_out, section_df, sec_numeric_cols, section_4326, units_4326, proximal_wells, units)


(_lines_gdf, _points_gdf, grid_gdf, inv_gdf, land_gdf, well_df, section_df,
 SEC_NUMERIC_COLS, section_enriched_4326, units_4326, proximal_wells, units_gdf) = load_data()
...
if valid_ep.any():
    ep_gdf = gpd.GeoDataFrame({"_pidx": ep_series[valid_ep].index, "geometry": ep_series[valid_ep].values}, crs=prospects.crs)
...
def analyze_prospects(pros, prox, sections, buffer_m):
    ...
    proximal_uwis = well_hits.groupby("index_right")["UWI"].apply(lambda x: ", ".join(x.astype(str))).reindex(pros.index, fill_value="")
    ...
    sec_join = gpd.sjoin(sections[["geometry", SECTION_OOIP_COL, SECTION_ROIP_COL]], buffer_gdf, how="inner", predicate="intersects")
    ...
    if len(field) >= 2:
        ...
        if all(m is not None for m in [eur_model, ip90_model, y1_model]):
            ...
            if not pros_cls.empty:
                ...
                pros_cls["Classification"] = pros_cls.apply(lambda r: classify_quadrant(r["Productivity_Z"], r["Resource_Z"], prod_threshold, resource_threshold), axis=1)
...
col_m4.metric("Classified", int(prospects["Classification"].notna().sum()) if "Classification" in prospects.columns else 0)
...
@st.fragment
def render_map():
    m = folium.Map(location=[clat, clon], zoom_start=11, tiles="CartoDB positron", control_scale=True)
    MiniMap(toggle_display=True).add_to(m)

    base_fg = folium.FeatureGroup(name="Existing Wells", show=True)
    lines_data = safe_geojson(_lines_gdf.to_crs(4326), 30)
    if lines_data:
        folium.GeoJson(lines_data, style_function=lambda _: {"color": "#d62728", "weight": 2, "opacity": 0.9}).add_to(base_fg)
    points_data = safe_geojson(_points_gdf.to_crs(4326), 0)
    if points_data:
        folium.GeoJson(points_data, style_function=lambda _: {"color": "#1f77b4", "weight": 1, "opacity": 0.95, "fillOpacity": 0.9}).add_to(base_fg)
    base_fg.add_to(m)

    inv_fg = folium.FeatureGroup(name="Inventory", show=True)
    inv_data = safe_geojson(inv_4326, 60)
    if inv_data:
        folium.GeoJson(inv_data, style_function=lambda _: {"color": "#999", "weight": 1, "fillOpacity": 0.06}).add_to(inv_fg)
    inv_fg.add_to(m)

    land_fg = folium.FeatureGroup(name="Land", show=True)
    land_data = safe_geojson(land_gdf.to_crs(4326), 100)
    if land_data:
        folium.GeoJson(land_data, style_function=lambda _: {"fillColor": "#fff9c4", "color": "#fff9c4", "weight": 0.5, "fillOpacity": 0.2}).add_to(land_fg)
    land_fg.add_to(m)

    units_fg = folium.FeatureGroup(name="Units", show=True)
    units_data = safe_geojson(units_gdf.to_crs(4326), 50)
    if units_data:
        folium.GeoJson(units_data, style_function=lambda _: {"color": "black", "weight": 2, "fillOpacity": 0}).add_to(units_fg)
    units_fg.add_to(m)

    section_fg = folium.FeatureGroup(name="Section Grid", show=False)
    sec_data = safe_geojson(section_enriched_4326, 80)
    if sec_data:
        folium.GeoJson(sec_data, style_function=lambda _: {"fillOpacity": 0, "color": "#999", "weight": 0.3}).add_to(section_fg)
    section_fg.add_to(m)

    buf_fg = folium.FeatureGroup(name="Buffers", show=False)
    for _, row in buffer_gdf_display.iterrows():
        if row.geometry and not row.geometry.is_empty:
            folium.GeoJson(row.geometry.__geo_interface__, style_function=lambda _: {"color": "#000", "weight": 1, "opacity": 0.4, "dashArray": "5,5"}).add_to(buf_fg)
    buf_fg.add_to(m)

    prospect_fg = folium.FeatureGroup(name="Prospects", show=True)
    for _, row in p_lines_4326.iterrows():
        if row.geometry and not row.geometry.is_empty:
            color, weight = row["_line_color"], 5 if row["_is_custom"] else 3
            folium.GeoJson(
                row.geometry.__geo_interface__,
                style_function=lambda _, c=color, w=weight: {"color": c, "weight": w, "opacity": 0.9},
                tooltip=folium.Tooltip(row["_tooltip"]),
            ).add_to(prospect_fg)
...
if classification_ready:
    ...
    if "Productivity_Z" in prospects.columns and "Resource_Z" in prospects.columns:
        ...
        if not scatter_df.empty:
            ...
            for cls_name, color in COLOR_MAP_CLASS.items():
                ...
                if not subset.empty:
                    fig2.add_trace(go.Scatter(
                        x=subset["Resource_Z"], y=subset["Productivity_Z"],
            ...
            fig2.update_layout(
                xaxis_title="Resource Z (σ)", yaxis_title="Productivity Z (σ)",
                plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                height=450, margin=dict(l=40, r=40, t=20, b=60),
...
if p_display.empty:
    ...
    df = p_display[table_cols].sort_values(
        "Proximal_Count" if "Proximal_Count" in p_display.columns else table_cols[0], ascending=False
    ).reset_index(drop=True)
    df.rename(columns={"_is_custom": "Custom Well"}, inplace=True)
    st.dataframe(df, use_container_width=True, height=400)
    st.download_button("📥 Download CSV", data=df.to_csv(index=False), file_name="prospects_classified.csv", mime="text/csv")
